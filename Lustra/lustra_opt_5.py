...
def check_if_mirrors_works(mirrors_positions, start_from: tuple[int,int], vector:int, not_visited):
    ...
    while True:
        ...
        if not ((0 <= row < garden_size) and (0 <= col < garden_size)):
            return visited_fields_between_last_mirror_and_border

        current_angle: int =  mirrors_positions.get((row, col), 0)
...
if __name__ == '__main__':
    ...
    not_visited_mirrors = mirrors.copy()

    visited_fields_1 = check_if_mirrors_works(mirrors, start_position, 1, not_visited_mirrors)
    visited_fields_2 = check_if_mirrors_works(mirrors, meta_position, -1, not_visited_mirrors)

    if visited_fields_1 and visited_fields_2 and len(not_visited_mirrors) == 1:

        field_to_move_mirror = visited_fields_1.intersection(visited_fields_2)
        if len(field_to_move_mirror) == 1:
            key = next(iter(not_visited_mirrors))
